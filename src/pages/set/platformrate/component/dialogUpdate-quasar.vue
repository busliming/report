 ...
 * @LastEditors: Nice
 * @LastEditTime: 2020-02-25 14:57:46
 -->
<template>
  <div
          ...
          <div>
            ...
            <div class="append-handle-btn-input row">
              <q-input
                ...
                color="panda-text-light"
                class="text-panda-text-dark panda-input-dense input-height-36px"
                v-model.trim="detailObj.remarks"
                placeholder="请输入备注"
              ></q-input>
            </div>
          </div>
...
export default {
  ...
  created() {
    this.formData.rangeAmountBegin = this.detailObj.rangeAmountBegin / 10000;
    this.formData.rangeAmountEnd = this.detailObj.rangeAmountEnd / 10000;
  ...
  methods: {
    ...
    handleSave() {
      ...
      if (
        ...
        if(!this.detailObj.paymentCycle){
          this.$message.error('缴纳周期不能为空！');
          return false;
        }
        // console.log(this.formData.rangeAmountBegin,this.formData.rangeAmountEnd*1);
        let params = {
          id: this.detailObj.id,
          computingStandard: this.detailObj.computingStandard, //计算标准
          rangeAmountBegin: this.formData.rangeAmountBegin * 10000, //金额范围开始
          rangeAmountEnd: this.formData.rangeAmountEnd * 10000, //金额范围结束
          terraceRate: this.detailObj.terraceRate, //平台费率
          paymentCycle: this.detailObj.paymentCycle, //缴纳周期
          remarks: this.detailObj.remarks //备注
        };
