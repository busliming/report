 ...
 * @LastEditors: Nice
 * @LastEditTime: 2020-08-12 15:48:10
 -->
<template>
  <div class="full-content">
    <q-card class="bg-white text-black dialog_main" style="width:825px; ">
          ...
          <div slot="orderAmount" slot-scope="text, record">
            <div v-for="(item, index) in record.expanded" :key="index">
              <div
                ...
                class="item-open"
              >
                <div>{{ item.orderAmount | amount }}</div>
              </div>
            </div>
          </div>
            ...
            <div v-for="(item, index) in record.expanded" :key="index">
              ...
              >
                <div :class="[item.profitRate < 0 ? 'red' : '']">
                  <span v-if="item.profitRate<0 && item.profitRate>-1">{{item.profitRate*100 | moneyFormat}}%</span>
...
export default {
  ...
  methods: {
    tableSortBy(pagination, filters, sorter) {
    ...
    initTableData() {
      ...
      api_data.post_admin_match_queryPlayStatisticList(params).then(res => {
