<template>
  <div class="full-width full-height" style="background: #F0F2F5">
    <div class="pl20x bg-panda-bg-6 mt20x pb20x row">
      <div class="append-handle-btn-input row line-height-30px" style="margin-top:1px;">
        <div class="mr10x fs12">账变类型</div>
      ...
      <div class="append-handle-btn-input pl30x height-30px line-height-30px">
        <a-button type="primary" style="height: 30px; line-height: 30px;" @click="handleSearch">搜索</a-button>
      </div>
    </div>
...
<script>
import { api_data, api_export, api_account } from "src/api/index.js";
...
import { userAccountConfig } from "src/pages/account/betting/config/userAccountConfig.js";
...
export default {
  ...
  methods: {
    ...
    initTableData() {
      ...
      api_account
        .post_admin_userReport_queryUserTransferList(params)
        .then(res => {
    ...
    handleExportExcel() {
      if (this.pagination.total > 0) {
        ...
        Object.assign(
          ...
          { url: "/admin/userReport/exportUserOrder" }
        );
