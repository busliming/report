 ...
 * @LastEditors: Nice
 * @LastEditTime: 2020-08-11 20:14:33
 -->
<template>
  <div
    ...
    <q-card class="bg-white text-black">
      ...
      >
        <span slot="merchantName" slot-scope="text, record">
          <span class="text-over-130 cursor-pointer" @click="handleCopy(record.merchantName,'商户名称')">{{ record.merchantName }}</span>
        </span>
        ...
        <a slot="action" slot-scope="text, record, index" class="q-gutter-md">
          ...
          <a-tooltip  placement="top" v-if="detailObj.childConnectMode == 1">
            <template slot="title">
...
import { subConfig } from "src/pages/account/mykey/config/subConfig.js";
...
import submixin from "src/pages/account/mykey/mixin/submixin.js";
export default {
  ...
  methods: {
    ...
    initTableData() {
      ...
      api_account
        .get_admin_merchant_queryKeyList(params)
        .then(res => {
