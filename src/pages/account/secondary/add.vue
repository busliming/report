  ...
  <div class="full-width full-height">
    <div class="pl20x mt20x">
      <q-breadcrumbs separator=">" active-color="whiddte">
        ...
        <q-breadcrumbs-el label="二级商户管理" class="text-panda-text-4" />
            ...
            <div class="row">
              <div class="left mt20x pl40x">
                ...
                <div class="flex">
                  ...
                  <div class="ml40x">
                    <div class="text-panda-text-7 mb10x fw_600">渠道商户</div>
                    <div class="append-handle-btn-input row" id="parentName">
                      <a-form-item class="w-130" has-feedback>
                        <span class="text-panda-text-3 mt10x">{{detail.merchantName}}</span>
                      </a-form-item>
                    </div>
                  </div>
                </div>
                ...
                <div class="append-handle-btn-input position-relative w-180">
                  <a-form-item>
                    <a-select
                      ...
                      :disabled="detail.childConnectMode === 2"
                      size="large"
...
import dialogSelectChannel from "src/pages/account/secondary/component/dialogSelectChannel.vue";
export default {
  ...
  methods: {
    initMerchantDetail() {
      ...
      api_account.get_admin_merchant_getDetail(params).then(res => {
        ...
        if (code == "0000000") {
          ...
          console.warn(this.detail, "detail");
          // this.initSetValues(detail);
          this.initChannelMerchant();
        } else {
    ...
    initChannelMerchant() {
      console.warn(this.detail, "detail");
      if (this.detail) {
        this.formData.transferMode = this.detail.transferMode; //控制url是否显示
        let initValues = {
          transferMode: this.detail.transferMode,
          url: this.detail.url,
          callbackUrl: this.detail.callbackUrl,
          balanceUrl: this.detail.balanceUrl,
          whiteIp: this.detail.whiteIp
        };
        if(this.formData.transferMode === 2){
