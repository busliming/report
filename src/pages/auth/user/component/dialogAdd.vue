 ...
 * @LastEditTime: 2020-09-11 20:06:41
 -->
<template>
    ...
    <q-card class="bg-white text-black">
      <q-card-section class="no-padding">
        <div class="row line-height-40px fs14 bg-panda-dialog text-panda-text-7 pr10x">
          <div class="pl20x fw_600">{{$t('auth_add_btn')}}</div>
          <q-space></q-space>
          ...
          <a-form-item>
            <div class="flex" style="width:165px; position: relative">
              <div
                ...
                :style="formData.avatar ? 'border: 1px dashed #c1c1c1' : ''"
              >
                <img
                  :src="httpUrl_72"
                  v-if="httpUrl_72"
                ...
                <a-form-item class="w-240" has-feedback>
                  <a-input
                    ...
                    v-decorator="[
                      'password',
                      { rules: [
                        { required: true, message: $t('auth_new_user.password_er') },
                      ]}
                    ]"
                  />
                </a-form-item>
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          values.avatar = this.formData.avatar;
          values.password = values.password;
          console.log(values, "values");
          this.loading = true
          let params = this.delete_empty_property_with_exclude(values);
          api_auth.post_admin_user_add(values).then(res => {
            this.loading = false
            let { code, msg, data } = res.data;
            if (code == "0000000") {
              this.$emit("closeDialog");
              this.$message.success(this.$t('auth_new_user_tip1'));
            } else {
