 ...
 * @LastEditors: Nice
 * @LastEditTime: 2020-05-26 10:42:09
 -->
<template>
  <vue-seamless :data="newsList" class="seamless-warp" :class-option="classOption">
    <ul class="item fs14" style="min-width: 560px;">
      <li v-for="(item,index) in newsList" :key="'news'+index" class="line-height-40px fs14 cursor-pointer">
...
export default {
  mixins: [commonmixin, financesorter],
  data() {
    return {
      newsList: [],
    };
  ...
  methods: {
    ...
    initLightNews() {
      api_message.post_admin_noticeNew_getLightNews().then(res => {
