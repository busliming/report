...
let env = {
  ...
  API_PREFIX_7: JSON.stringify("yewu17"), //#业务  业务商户-分控(merchant-admin) 10714
  API_PREFIX_8: JSON.stringify("yewu18"), //#业务  业务商户-分控(merchant-admin) 10714
...
if (current_env == "local_local") {
  ...
  let proxy = {
    ...
    "/yewu17": {
      target: `${current_request_domain}/yewu17`,
      changeOrigin: true,
      pathRewrite: {
        "^/yewu17": ""
      }
    ...
    "/yewu18": {
      target: `${current_request_domain}/yewu18`,
      changeOrigin: true,
      pathRewrite: {
        "^/yewu18": ""
      }
   ...
   */
  // env.API_PREFIX_FILE_REAL =  JSON.stringify(
  //   `${current_request_domain}/yewu9`
  // );

  // env.API_PREFIX_FILE_FINANCE =  JSON.stringify(
  //   `${current_request_domain}/yewu14`
  // );
      ...
      extendWebpack(cfg) {
        ...
        cfg.externals = [
          function (context, request, callback) {
            if (/v-charts\/lib\/line/.test(request)) {
              return callback(null,  ['VeIndex', 'VeLine'])
            }
            if (/v-charts\/lib\/ring/.test(request)) {
              return callback(null, ['VeIndex', 'VeBar'])
            }
            if (/v-charts\/lib\/bar/.test(request)) {
              return callback(null, ['VeIndex', 'VeRing'])
            }
            if (/v-charts/.test(request)) {
              return callback(null, 'VeIndex')
            }
            if (/echarts/.test(request)) {
              return callback(null, 'echarts')
            }
            if (/jspdf/.test(request)) {
              return callback(null, 'jsPDF')
            }
            if (/^vue$/.test(request)) {
